...
class Vision2TextAttention(nn.Module):
    ...
    def forward(
        ...
        bsz1, tgt_len1, embed_dim1 = vision_hidden_states.size()
        
        # print(f"bsz: {bsz}, tgt_len: {tgt_len}, embed_dim: {embed_dim}")
        # print(f"bsz1: {bsz1}, tgt_len1: {tgt_len1}, embed_dim1: {embed_dim1}")
...
class Vision2TextAttOnlyLayer(nn.Module):
    def __init__(self, config, layer: int = None, ):
        super().__init__()
        self.embed_dim = config.hidden_size
        # self.args = args
        self.layer = layer
        self.self_attn = Vision2TextAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim)
...
class VisualLayerWiseMoE(nn.Module):
    def __init__(self, config, aggred_layer_num=1):
        ...
        # self.output_path="./text_expert_weights.txt"
    ...
    def forward(self,
        ...
        # Select layers from 0 to 12
        selected_visual_layers = [layer_wise_visual_hidden_states[i] for i in range(13)]
        expert_router_input = torch.cat([text_hidden_states] + selected_visual_layers, dim=-1)
        
        expert_weights = self.expert_router(expert_router_input)
        expert_weights = torch.softmax(expert_weights, dim=-1)
        
        # 保存 expert_weights 到文件
        # self.save_expert_weights(expert_weights)
                
        bsz, tgt_len, embed_dim = text_hidden_states.size()
    
        
        selected_hidden_states = [layer_wise_visual_hidden_states[i] for i in range(13)]
        layer_wise_visual_hidden_states = torch.stack(selected_hidden_states, dim=2)
    ...
    def save_expert_weights(self, expert_weights):
        """
        :param expert_weights: (batch_size, num_experts + 1) torch.Tensor
        """
        with open(self.output_path, "a") as f:
            for sample_weights in expert_weights:  
                sorted_indices = torch.argsort(sample_weights, descending=True)
                top_13_indices = sorted_indices[:13].tolist()
                f.write(" ".join(map(str, top_13_indices)) + "\n")
...
class Vision2TextLayerWiseAgg(nn.Module):
    def __init__(self, config, aggred_layer_num=1):
        ...
        self.cross_modal_att_layer = nn.ModuleList(
            [Vision2TextAttOnlyLayer(config, layer=i) for i in range(aggred_layer_num)])
    ...
    def forward(
        ...
        if output_attentions:
            #新增
            max_dim = max(tensor.shape[-1] for tensor in all_cross_modal_attentions)
            all_cross_modal_attentions = [F.pad(tensor, (0, max_dim - tensor.shape[-1])) for tensor in all_cross_modal_attentions]
        ...
        # # 如果 outputs 是一个元组，打印每个元素的形状
        # if isinstance(outputs, tuple):
        #     for i, output in enumerate(outputs):
        #         if hasattr(output, 'shape'):
        #             print(f"outputs[{i}].shape:", output.shape)
        #         else:
        #             print(f"outputs[{i}] is not a tensor, type: {type(output)}")
        # else:
        #     print("outputs.shape:", outputs.shape)
        # """
        # outputs[0].shape: torch.Size([32, 80, 768])
        # """
        # exit()
...
#新加
class Text2VisionAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config):
        ...
        #     config.attention_probs_dropout_prob = 0.1  # 设置一个默认值
        # self.dropout = config.attention_probs_dropout_prob
        self.dropout = config.attention_dropout  # only for vision config
        
        self.vision_q_proj = nn.Linear(self.embed_dim, self.embed_dim)
        self.text_k_proj = nn.Linear(self.embed_dim, self.embed_dim)
        self.text_v_proj = nn.Linear(self.embed_dim, self.embed_dim)

        self.text_and_vision_k_proj = nn.Linear(2 * self.embed_dim, self.embed_dim)
    ...
    def forward(
            ...
            output_attentions: bool = False,
            current_layer: int = None,
            vision2text_hidden_states: torch.Tensor = None,
    ):
        bsz, tgt_len, embed_dim = vision_hidden_states.size()
        bsz1, tgt_len1, embed_dim1 = text_hidden_states.size()

        query_states = self.vision_q_proj(vision_hidden_states) * self.scale
        key_states = self._shape(self.text_k_proj(text_hidden_states), -1, bsz)
        value_states = self._shape(self.text_v_proj(text_hidden_states), -1, bsz)

        if vision2text_hidden_states is not None:
            key_states = self._shape(
                self.text_and_vision_k_proj(torch.cat((vision2text_hidden_states, vision2text_hidden_states), dim=-1)),
                -1,
                bsz)
...
class Text2VisionAttOnlyLayer(nn.Module):
    def __init__(self, config, layer: int = None, ):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.layer = layer
        self.self_attn = Text2VisionAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim)

    def forward(
            self,
            vision_hidden_states: torch.Tensor,
            all_text_hidden_states: Tuple,
            output_attentions: bool = False,
    ):
        text_hidden_states = all_text_hidden_states[self.layer]
        attentional_hidden_states, attn_weights = self.self_attn(
            vision_hidden_states=vision_hidden_states,
            text_hidden_states=text_hidden_states,
            output_attentions=output_attentions,
...
class TextLayerWiseMoE(nn.Module):
    def __init__(self, config, aggred_layer_num=1):
        super().__init__()
        self.config = config
        self.n_experts = 13
        ...
        # self.output_path="./visual_expert_weights.txt"

    def forward(self,
                vision_hidden_states: torch.Tensor = None,
                layer_wise_text_hidden_states: Tuple = None,
                attention_mask: torch.Tensor = None, ):

        # Select layers from 0 to 12
        selected_text_layers = [layer_wise_text_hidden_states[i] for i in range(13)]
        expert_router_input = torch.cat([vision_hidden_states] + selected_text_layers, dim=-1)
        ...
        selected_hidden_states = [layer_wise_text_hidden_states[i] for i in range(13)]
        layer_wise_text_hidden_states = torch.stack(selected_hidden_states, dim=2)

        input_hidden_states = torch.cat((vision_hidden_states.unsqueeze(2).expand(layer_wise_text_hidden_states.shape),
                                         layer_wise_text_hidden_states), dim=-1)
        experts_out = torch.einsum('bsni,nio->bsno', input_hidden_states, self.experts1)
        experts_out = self.activation_fn(experts_out)
        experts_out = torch.einsum('bsni,nio->bsno', experts_out, self.experts2)
        vision_hidden_states = vision_hidden_states.unsqueeze(2)
        experts_out = experts_out + vision_hidden_states.expand(layer_wise_text_hidden_states.shape)
        experts_out = torch.cat([vision_hidden_states, experts_out], dim=2)
        experts_out = torch.einsum('bsno,bsn->bsno', experts_out, expert_weights)
        moe_fusion = torch.sum(experts_out, dim=2)
...
class Text2VisionLayerWiseAgg(nn.Module):
    def __init__(self, config, aggred_layer_num=1):
        super().__init__()
        self.cross_modal_att_layer = nn.ModuleList(
            [Text2VisionAttOnlyLayer(config, layer=i) for i in range(aggred_layer_num)])
        self.moe_layer_wise_agg = TextLayerWiseMoE(config, aggred_layer_num)
        self.aggred_layer_num = aggred_layer_num
    ...
    def forward(
        ...
        layer_wise_text_hidden_states = ()
        all_cross_modal_attentions = () if output_attentions else None
        vision_hidden_states = all_vision_hidden_states[-1]
        detached_text_hidden_states = ()
        for i in range(self.aggred_layer_num):
            detached_text_hidden_states += (all_text_hidden_states[i].detach(),)
        for i in range(self.aggred_layer_num):
            attention_outputs = self.cross_modal_att_layer[i](vision_hidden_states,
                                                              detached_text_hidden_states,
                                                              output_attentions)
            if output_attentions:
                all_cross_modal_attentions += (attention_outputs[1],)
            layer_wise_text_hidden_states += (attention_outputs[0],)
        cross_modal_hidden_state = self.moe_layer_wise_agg(vision_hidden_states=vision_hidden_states,
                                                           layer_wise_text_hidden_states=layer_wise_text_hidden_states,
                                                           attention_mask=attention_mask)
        outputs = (cross_modal_hidden_state,)
...
class BertVitInterBaseModel(nn.Module):
    def __init__(self, vision_config, text_config, add_pooling_layer=False):
        ...
        # and self.args.device == 'cuda' else 'cpu'
